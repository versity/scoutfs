...
int check_supers(void)
{
	struct scoutfs_super_block *super = NULL;
	struct block *blk = NULL;
	struct scoutfs_quorum_slot *slots = NULL;
	struct in_addr in;
	uint16_t family;
	uint16_t port;
	int ret;
	...
	if ((global_super->flags != SCOUTFS_FLAG_IS_META_BDEV) && (global_super->flags != 0))
	...
	slots = global_super->qconf.slots;
	for (int i = 0; i < SCOUTFS_QUORUM_MAX_SLOTS; i++) {
		family = le16_to_cpu(slots[i].addr.v4.family);
		port = le16_to_cpu(slots[i].addr.v4.port);
		in.s_addr = htonl(slots[i].addr.v4.addr);

		if (family == SCOUTFS_AF_NONE) {
			debug("Quorum slot %u is empty", i);
			continue;
		}
		else if (family != SCOUTFS_AF_IPV4)
			problem(PB_QSLOT_BAD_FAM, "Quorum Slot %u has invalid family: %u", i, family);

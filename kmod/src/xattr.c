			     ...
			     struct list_head *ind_locks)
{
	...
	if (tgs->indx) {
		if (value)
			ret = scoutfs_item_create(sb, &tag_key, NULL, 0, tag_lock);
		else
			ret = scoutfs_item_dirty(sb, &tag_key, tag_lock);
		if (ret < 0)
			goto out;
		undo_indx = true;
	}
	...
	if (tgs->indx && (found_parts && !value)) {
		err = scoutfs_item_delete(sb, &tag_key, tag_lock);
		BUG_ON(err); /* dirty deletion shouldn't fail */
	}
...
out:
	if (ret < 0 && undo_indx) {
		if (value)
			...
			err = scoutfs_item_create(sb, &tag_key, NULL, 0, tag_lock);
		       ...
		       struct scoutfs_lock *lock)
{
	...
	for (;;) {
		...
		if (tgs.indx) {
			ret = scoutfs_item_delete(sb, &tag_key, tag_lock);
			if (ret < 0)
				goto out;
		}

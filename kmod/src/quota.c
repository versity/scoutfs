...
static void insert_cached_check(struct squota_info *qtinf, struct squota_input *inp, int result)
{
	...
	chk = kmalloc(sizeof(struct squota_check), GFP_NOFS);
	if (!chk)
		return;
...
/*
 * A rule matches input as long as all of the rule's source selections
 * match the input's attrs.  The rule might not have any selections.
 */
...
{
	/* zero full size for hash memcmp */
...
int scoutfs_quota_setup(struct super_block *sb)
{
	...
	KC_REGISTER_SHRINKER(&qtinf->shrinker, "scoutfs-quota_info:" SCSBF, SCSB_ARGS(sb));

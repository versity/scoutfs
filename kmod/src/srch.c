...
static void append_padded_entry(struct scoutfs_srch_file *sfl,
				...
				int len)
{
	put_unaligned_le16(len-2, srb->entries + le32_to_cpu(srb->entry_bytes));
				...
				struct scoutfs_srch_block *srb)
{
	...
	sre.hash = srb->tail.hash;
	sre.ino = srb->tail.ino;
	sre.id = srb->tail.id;

	target = SCOUTFS_SRCH_BLOCK_SAFE_BYTES + 2;

	while ((diff = target - le32_to_cpu(srb->entry_bytes)) > 0) {
		append_padded_entry(sfl, srb, &sre, 10);
		if (diff % 20 == 0) {
			append_padded_entry(sfl, srb, &sre, 10);
		} else {
			append_padded_entry(sfl, srb, &sre, 9);
		}
	}
